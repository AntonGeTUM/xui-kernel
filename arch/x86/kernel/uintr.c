 ...
 * API to free them up
 */
static void do_uintr_unregister_vector(u64 uvec,
				       struct uintr_upid_ctx *upid_ctx)
{
...
static int do_uintr_register_vector(u64 uvec,
				    struct uintr_upid_ctx **uvecfd_upid_ctx)
{
	struct uintr_upid_ctx *upid_ctx;
...
static void uvecfd_show_fdinfo(struct seq_file *m, struct file *file)
{
	...
	seq_printf(m, "uintr: receiver: %d vector:%llu\n",
		   uvecfd_ctx->upid_ctx->task->pid, uvecfd_ctx->uvec);
}
#endif
...
static inline void mark_uitte_invalid(struct uintr_uitt_ctx *uitt_ctx,
...
bool uintr_check_uitte_valid(struct uintr_uitt_ctx *uitt_ctx,
			     unsigned int entry)
{
...
static int do_uintr_unregister_sender(struct uintr_uitt_ctx *uitt_ctx,
				      unsigned int entry)
{
	/* Check if the supplied UITT index is valid */
	if (!uintr_check_uitte_valid(uitt_ctx, entry)) {
		pr_debug(
			"send: Unregister for invalid UITTE %d for uitt_ctx=%lx\n",
...
static long uipifd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case UIPI_SET_TARGET_TABLE:
		...
		if (t->mm->context.uitt_ctx) {
			pr_debug(
				"send: uitt_ctx is already set in mm for task=%d UITT=%px\n",
		...
		pr_debug(
			"send: uitt_ctx is being set in mm for task=%d UITT=%px\n",
			t->pid, uitt_ctx->uitt);
		break;
...
static int uipifd_open(struct inode *inode, struct file *file)
{
	...
	if (!uitt_ctx) {
		...
		pr_debug(
			"send: uitt_ctx already set in mm for task=%d UITT=%px\n",
			current->pid, uitt_ctx->uitt);
	}
...
SYSCALL_DEFINE2(uintr_unregister_sender, int, uipi_index, unsigned int, flags)
{
	...
	if (!uitt_ctx) {
		pr_debug(
			"send: unregister sender for task=%d, something might be wrong here\n",
			current->pid);
		ret = -EINVAL;
		goto out_fdput;
	}
		...
		flags)
{
	...
	//if (!cpu_feature_enabled(X86_FEATURE_UINTR)) {
	//	pr_info(KERN_INFO "NOT ENABLED\n");
	//	return -ENOSYS;
	//}

	pr_info("recv: requesting register handler task=%d flags %d handler %lx\n",
 ...
 */
SYSCALL_DEFINE3(uintr_alt_stack, void __user *, sp, size_t, size, unsigned int,
		flags)
{
	int ret;
...
static int uintr_notify_receiver(u64 uvec, struct uintr_upid_ctx *upid_ctx)
{
	...
	pr_debug("notify: puir=%llx SN %x ON %x NDST %x NV %x", upid->puir,
		 test_bit(UINTR_UPID_STATUS_SN,
			  (unsigned long *)&upid->nc.status),
		 test_bit(UINTR_UPID_STATUS_ON,
			  (unsigned long *)&upid->nc.status),
		 upid->nc.ndst, upid->nc.nv);

	/* TODO: Use cmpxchg for UPID since we are doing read-modify-write */
	if (!test_bit(UINTR_UPID_STATUS_SN,
		      (unsigned long *)&upid->nc.status) &&
	    !test_and_set_bit(UINTR_UPID_STATUS_ON,
			      (unsigned long *)&upid->nc.status)) {
...
{
	set_bit(UINTR_UPID_STATUS_BLKD,
		(unsigned long *)&upid_ctx->upid->nc.status);
	upid_ctx->waiting = true;
}
...
void switch_uintr_prepare(struct task_struct *prev)
{
	 ...
	 */
	if (IS_ENABLED(CONFIG_X86_UINTR_BLOCKING) &&
	    is_uintr_waiting_enabled(prev) && task_is_interruptible(prev)) {
...
{
	upid_ctx->waiting = false;
	clear_bit(UINTR_UPID_STATUS_BLKD,
		  (unsigned long *)&upid_ctx->upid->nc.status);
}
...
{
	if (IS_ENABLED(CONFIG_X86_UINTR_BLOCKING) && is_uintr_receiver(next) &&
